...
export async function generateBlogArticle(englishTitle: string, englishContent: string): Promise<GeneratedContent | null> {
    const prompt = `
あなたはプロのIT・AI系ブログライターです。
以下の「英語記事のタイトル」と「内容」を元に、日本の読者が興味を持つようなブログ記事を作成してください。

指示事項:
1. 日本語のキャッチーな「タイトル」をつけること（30文字前後）
2. 記事の「要約（結論）」を3〜4行でまとめること
3. 本文は、「背景」「詳細」「影響やまとめ」など見出しをつけて論理的に解説すること
4. 専門用語はなるべく分かりやすく噛み砕くこと
